...
const LOG: LogStep[] = [
  { id: 'tc-1', type: 'tool_call', seq: 1, name: 'market_data', input: {}, ok: true, ms: 312 },
  { id: 'tc-2', type: 'tool_call', seq: 2, name: 'news_search', input: {}, ok: true, ms: 511 },
];

describe('AgentLogStrip', () => {
  it('shows full strip while streaming', () => {
    render(<AgentLogStrip log={LOG} usage={null} status="streaming" model="gemini" />);
    // The steps container should be present
    expect(screen.getByRole('region', { name: /agent log/i })).toBeInTheDocument();
  ...
  it('collapses to summary bar when status becomes success', () => {
    render(<AgentLogStrip log={LOG} usage={USAGE} status="success" model="gemini" />);
    ...
    expect(screen.getByRole('button', { name: /expand agent log/i })).toBeInTheDocument();
  ...
  it('clicking collapse button in expanded view returns to summary', () => {
    ...
    // Then collapse
    const collapseBtn = screen.getByRole('button', { name: /collapse agent log/i });
    fireEvent.click(collapseBtn);
    // Back to summary
    expect(screen.getByRole('region', { name: /agent log summary/i })).toBeInTheDocument();
  });

  it('does not collapse when status is error (full strip stays visible)', () => {
    render(<AgentLogStrip log={LOG} usage={USAGE} status="error" model="gemini" />);
    // Should NOT show the collapsed summary
    expect(screen.queryByRole('button', { name: /expand agent log/i })).toBeNull();
    // Full strip visible — aria-label is exactly "Agent log" (no "summary" suffix)
    expect(screen.getByRole('region', { name: 'Agent log' })).toBeInTheDocument();
  });
});

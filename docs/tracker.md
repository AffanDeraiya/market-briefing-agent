...
**Started:** 2026-06-13  ·  **Target ship:** _(date)_  ·  **Current phase:** 6 (harness complete; awaiting live cassette corpus) → 7 · **active branch:** `working` (master = final; merge after full testing)
...
- [ ] 6–8 recorded runs committed — **1 of ~6** (`aapl_3mo`); remaining need live Gemini runs (user-gated). `evals/record_cassettes.py` ready (`make eval-live`).
- [x] Citation faithfulness checker ≥95% → record: **100%** (1 cassette) — `checks.citation_faithfulness` (citation grounded in recorded tool_io URLs or kind=tool)
- [x] Structure compliance ≥98% → record: **100%** (1 cassette) — `checks.structure_compliance` (8-point checklist)
